  ...
  upload.single('photo')(req, res, (err) => {
    if (err instanceof multer.MulterError) {
      if (err.code === 'LIMIT_FILE_SIZE') {
        req.session.flash = { type: 'error', message: 'Ukuran file terlalu besar. Maksimal 2MB.' };
      } else {
        req.session.flash = { type: 'error', message: `Upload error: ${err.message}` };
      }
...
router.post('/:id/update',  isAuthenticated, uploadMiddleware, asetController.update);
