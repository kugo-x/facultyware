...
const index = async (req, res, next) => {
  try {
    ...
    res.render('ruangan/index', {
      ...
      ruangans, stats, buildings, q, buildingId, flash,
...
const store = async (req, res, next) => {
  try {
    const { room_name, room_code, floor, capacity, is_public, building_id } = req.body;
    ...
    if (errors.length > 0) {
      ...
      return res.render('ruangan/create', {
        title: 'Tambah Ruangan',
        buildings, errors, old: req.body,
        user: req.session.username, flash: null,
      });
    }
    ...
    await RuanganModel.create({
      ...
      floor, capacity: parseInt(capacity) || 0,
    ...
    if (err.code === 'ER_DUP_ENTRY') {
      req.session.flash = { type: 'error', message: 'Kode ruangan sudah digunakan.' };
...
const edit = async (req, res, next) => {
  try {
    ...
    if (!ruangan) {
      req.session.flash = { type: 'error', message: 'Ruangan tidak ditemukan.' };
...
const update = async (req, res, next) => {
  try {
    ...
    if (photo) {
      ...
      if (existing?.photo) {
        const oldPath = path.join(__dirname, '../public/uploads/ruangan', existing.photo);
...
const destroy = async (req, res, next) => {
  try {
    ...
    // Hapus foto jika ada
    if (ruangan.photo) {
      const filePath = path.join(__dirname, '../public/uploads/ruangan', ruangan.photo);
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    }

    // Cascade delete (assets akan terhapus via FK ON DELETE CASCADE)
    await RuanganModel.destroy(id);
    req.session.flash = { type: 'success', message: `Ruangan "${ruangan.room_name}" beserta seluruh asetnya berhasil dihapus.` };
    res.redirect('/ruangan');
  } catch (err) { next(err); }
};
...
const exportPdf = async (req, res, next) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="laporan-ruangan.pdf"');
    doc.pipe(res);

    doc.fontSize(16).font('Helvetica-Bold').text('Laporan Daftar Ruangan', { align: 'center' });
    ...
    const headers = ['#', 'Nama Ruangan', 'Kode', 'Gedung', 'Lantai', 'Kapasitas', 'Publik', 'Jumlah Aset'];
    const startX  = 30;
    let y = doc.y;
    const rowH = 20;
    ...
    ruangans.forEach((r, i) => {
      ...
      drawRow(cols, [
        i + 1, r.room_name, r.room_code, r.building_name,
        r.floor || '-', r.capacity, r.is_public ? 'Ya' : 'Tidak', r.jumlah_aset,
      ]);
    });
...
const exportDocx = async (req, res, next) => {
  try {
    ...
    const headerCols = ['No', 'Nama Ruangan', 'Kode', 'Gedung', 'Lantai', 'Kapasitas', 'Jumlah Aset'];
    ...
    const dataRows = ruangans.map((r, i) =>
      new TableRow({
        children: [
    ...
    const document = new Document({
      sections: [{
        children: [
          new Paragraph({ text: 'Laporan Daftar Ruangan', heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER }),
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="laporan-ruangan.docx"');
    res.send(buffer);
  } catch (err) { next(err); }
};
...
const asetIndex = async (req, res, next) => {
  try {
    ...
    res.render('ruangan/aset/index', {
      title: `Aset — ${ruangan.room_name}`,
      ruangan, asets, stats, flash,
      user: req.session.username,
      kondisiLabel, statusLabel, perolehanLabel,
    });
  } catch (err) { next(err); }
};
...
const asetCreate = async (req, res, next) => {
  try {
    ...
    res.render('ruangan/aset/create', {
      title: `Tambah Aset — ${ruangan.room_name}`,
      ruangan, grants, flash,
      user: req.session.username,
      errors: null, old: {},
    });
  } catch (err) { next(err); }
...
const asetStore = async (req, res, next) => {
  try {
    ...
    const { asset_name, asset_code, acquisition_type, acquisition_date,
            acquisition_cost, asset_grant_id, condition, status } = req.body;
    ...
    if (errors.length > 0) {
      ...
      return res.render('ruangan/aset/create', {
        title: `Tambah Aset — ${ruangan.room_name}`,
        ruangan, grants, errors, old: req.body,
        user: req.session.username, flash: null,
      });
    }
    ...
    req.session.flash = { type: 'success', message: `Aset "${asset_name}" berhasil ditambahkan ke ruangan ini.` };
    res.redirect(`/ruangan/${id}/aset`);
  } catch (err) {
    if (err.code === 'ER_DUP_ENTRY') {
      req.session.flash = { type: 'error', message: 'Kode aset sudah digunakan.' };
...
const asetEdit = async (req, res, next) => {
  try {
    ...
    res.render('ruangan/aset/edit', {
      title: `Edit Aset — ${aset.asset_name}`,
      ruangan, aset, grants, flash,
      user: req.session.username,
      errors: null,
    });
  } catch (err) { next(err); }
...
const asetUpdate = async (req, res, next) => {
  try {
    ...
    if (!ruangan) return res.redirect('/ruangan');
    const aset = await AsetModel.getById(asetId);
    if (!aset || String(aset.room_id) !== String(id)) return res.redirect(`/ruangan/${id}/aset`);
    ...
    if (errors.length > 0) {
      ...
      return res.render('ruangan/aset/edit', {
        title: `Edit Aset — ${aset.asset_name}`,
        ruangan, aset: { ...aset, ...req.body, id: asetId },
        grants, errors, flash: null,
        user: req.session.username,
      });
    }
    ...
    req.session.flash = { type: 'success', message: `Aset "${asset_name}" berhasil diperbarui.` };
    res.redirect(`/ruangan/${id}/aset`);
  } catch (err) {
    if (err.code === 'ER_DUP_ENTRY') {
      req.session.flash = { type: 'error', message: 'Kode aset sudah digunakan.' };
      return res.redirect(`/ruangan/${req.params.id}/aset/${req.params.asetId}/edit`);
    }
    next(err);
  }
};
...
const asetDestroy = async (req, res, next) => {
  try {
    ...
    req.session.flash = { type: 'success', message: `Aset "${aset.asset_name}" berhasil dihapus.` };
    res.redirect(`/ruangan/${id}/aset`);
  } catch (err) { next(err); }
};
...
module.exports = {
  ...
  // Aset dalam Ruangan
  asetIndex, asetCreate, asetStore, asetEdit, asetUpdate, asetDestroy,
};
