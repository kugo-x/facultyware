...
// GET /gedung â€” Daftar gedung + search
...
// GET /gedung/create â€” Form tambah gedung
...
// POST /gedung/store â€” Simpan gedung baru
const store = async (req, res, next) => {
  try {
    ...
    req.session.flash = {
      type: "success",
      message: `Gedung "${name}" berhasil ditambahkan.`,
    };
    res.redirect("/gedung");
  } catch (err) {
    if (err.code === "ER_DUP_ENTRY") {
      req.session.flash = {
        type: "error",
        message: "Kode gedung sudah digunakan. Gunakan kode yang berbeda.",
      };
...
// GET /gedung/:id/edit â€” Form edit gedung
...
// POST /gedung/:id/update â€” Update gedung
const update = async (req, res, next) => {
  try {
    ...
    req.session.flash = {
      type: "success",
      message: `Gedung "${name}" berhasil diperbarui.`,
    };
    res.redirect("/gedung");
  } catch (err) {
    if (err.code === "ER_DUP_ENTRY") {
...
// POST /gedung/:id/delete â€” Hapus gedung
const destroy = async (req, res, next) => {
  try {
    ...
    req.session.flash = {
      type: "success",
      message: `Gedung "${gedung.name}" berhasil dihapus.`,
    };
    res.redirect("/gedung");
  } catch (err) {
    next(err);
...
const exportPdf = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="laporan-gedung.pdf"',
    );
    doc.pipe(res);
    ...
    doc
      .fontSize(18)
      .font("Helvetica-Bold")
      .text("Laporan Daftar Gedung", { align: "center" });
    doc
      .fontSize(11)
      .font("Helvetica")
    ...
    doc.text(`â€¢ Total Gedung  : ${stats.total_gedung}`);
    doc.text(`â€¢ Total Ruangan : ${stats.total_ruangan}`);
    doc.moveDown(1);
...
const exportDocx = async (req, res, next) => {
  try {
    ...
    const headerCells = ["No", "Nama Gedung", "Kode", "Deskripsi"].map(
      (text) =>
        new TableCell({
          children: [
            new Paragraph({
      ...
      sections: [
        {
          children: [
            ...
            new Paragraph({
              children: [
                new TextRun({
                  text: `Total Gedung: ${stats.total_gedung}   |   Total Ruangan: ${stats.total_ruangan}`,
                  bold: true,
                }),
              ],
            }),
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="laporan-gedung.docx"',
    );
    res.send(buffer);
  } catch (err) {
    next(err);
...
const apiShow = async (req, res, next) => {
  try {
    ...
    if (!gedung)
      return res
        .status(404)
        .json({ success: false, message: "Gedung tidak ditemukan." });
...
const apiCreate = async (req, res, next) => {
  try {
    ...
    if (!name || !code) {
      return res
        .status(422)
        .json({ success: false, message: "Field name dan code wajib diisi." });
    }
    const id = await GedungModel.create({
      name: name.trim(),
      code: code.trim(),
      description,
    });
    ...
    if (err.code === "ER_DUP_ENTRY") {
      return res
        .status(409)
        .json({ success: false, message: "Kode gedung sudah digunakan." });
    }
    next(err);
  }
};
  ...
  try {
    ...
    if (masihAdaRuangan) {
      return res
        ...
        .json({
          success: false,
          message: "Gedung masih memiliki data ruangan, tidak dapat dihapus.",
        });
    }
    await GedungModel.destroy(id);
