...
// GET /aset â€” Daftar aset ruangan
...
// POST /aset/store â€” Simpan aset ruangan baru
  ...
  try {
    ...
    if (err.code === "ER_DUP_ENTRY") {
      req.session.flash = {
        type: "error",
        message:
          "Kode aset ruangan sudah digunakan. Gunakan kode yang berbeda.",
      };
...
const edit = async (req, res, next) => {
  try {
    ...
    if (!aset) {
      req.session.flash = {
        type: "error",
        message: "Aset ruangan tidak ditemukan.",
      };
...
const destroy = async (req, res, next) => {
  try {
    ...
    req.session.flash = {
      type: "success",
      message: `Ruangan "${aset.room_name}" berhasil dihapus.`,
    };
    res.redirect("/aset");
  } catch (err) {
    next(err);
...
const exportPdf = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="laporan-aset-ruangan.pdf"',
    );
    doc.pipe(res);
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("Laporan Aset Ruangan", { align: "center" });
    doc
      .fontSize(10)
      .font("Helvetica")
    ...
    asets.forEach((a, i) => {
      ...
      drawRow(cols, [
        ...
        a.floor || "-",
        a.capacity,
        kondisiLabel[a.asset_condition] || a.asset_condition,
...
const exportDocx = async (req, res, next) => {
  try {
      ...
      (a, i) =>
        new TableRow({
          children: [
            ...
            new TableCell({
              children: [
                new Paragraph(statusLabel[a.asset_status] || a.asset_status),
              ],
            }),
            new TableCell({
              children: [
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="laporan-aset-ruangan.docx"',
    );
    res.send(buffer);
  } catch (err) {
    next(err);
...
const apiShow = async (req, res, next) => {
  try {
    ...
    if (!aset)
      return res
        .status(404)
        .json({ success: false, message: "Aset tidak ditemukan." });
...
// POST /api/aset  â€” body: { room_id, asset_name, asset_code, acquisition_type, acquisition_date, condition, status, ... }
const apiCreate = async (req, res, next) => {
  try {
    ...
    if (!asset_name?.trim()) missing.push("asset_name");
    if (!asset_code?.trim()) missing.push("asset_code");
    if (!acquisition_type) missing.push("acquisition_type");
    if (!acquisition_date) missing.push("acquisition_date");
    ...
    if (missing.length > 0) {
      return res
        ...
        .json({
          success: false,
          message: `Field wajib kurang: ${missing.join(", ")}`,
        });
    }

    const id = await AsetModel.create(room_id, {
      asset_name,
      asset_code,
      acquisition_type,
      acquisition_date,
      acquisition_cost: req.body.acquisition_cost || null,
      asset_grant_id: req.body.asset_grant_id || null,
      condition,
      status,
    });
    ...
    if (err.code === "ER_DUP_ENTRY") {
      return res
        .status(409)
        .json({ success: false, message: "Kode aset sudah digunakan." });
    }
    next(err);
  }
};
...
const apiUpdate = async (req, res, next) => {
  try {
    ...
    await AsetModel.update(id, {
      ...
      asset_grant_id:
        data.asset_grant_id !== undefined
          ? data.asset_grant_id
          : existing.asset_grant_id,
...
const apiDelete = async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      message: `Aset "${existing.asset_name}" berhasil dihapus.`,
    });
  } catch (err) {
    next(err);
  }
